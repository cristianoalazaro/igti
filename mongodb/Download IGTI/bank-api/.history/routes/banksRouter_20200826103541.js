...
router.get("/account/transferToPrivate/", async (req, res, next) => {
  try {
    const transferToPrivate = await Accounts.aggregate([
      {
        $group: {
          _id: "$agencia",
          balance: { $max: "$balance" },
    ...
    if (transferToPrivate.length === 0) {
      throw new Error("nenhuma conta apta para agencia Private");
    }
    transferToPrivate.forEach((toPrivate) => {
      const { _id, balance } = toPrivate;
      const newAccounts = new Accounts.find({
        agencia: _id,
        balance,
      });
      console.log("Antes" + newAccounts);
      newAccounts.agencia = 99;
      console.log("Depois" + newAccounts);
    });
