...
router.delete("/account/delete/", async (req, res, next) => {
  try {
    ...
    deleteAccount = await Accounts.find({ agencia: deleteAccount.agencia });
...
router.get("/account/transferToPrivate/", async (req, res, next) => {
  try {
    const transferToPrivates = await Accounts.aggregate([
      {
        $group: {
          _id: "$agencia",
          balance: { $max: "$balance" },
    ...
    for (const [index, transferToPrivate] of transferToPrivates.entries()) {
      console.log(transferToPrivate);
      const { _id, balance } = transferToPrivate;
      console.log(_id);
      console.log(balance);
      let newAccounts = await Accounts.find({
        agencia: _id,
        balance: balance,
      });
      console.log(`${index}`);
      console.log("Antes " + newAccounts[index + 1]);
      //newAccounts[index].agencia = 99;
      console.log("Depois " + newAccounts[index + 1]);
    }
