...
router.get("/account/checkBalance/", async (req, res, next) => {
  try {
    const account = req.body;
    const checkBalance = await validateAccount(account, next);
...
router.delete("/account/delete/", async (req, res, next) => {
    try {
      const account = req.body;
      let deleteAccount = await validateAccount(account, next);
      deleteAccount = new Accounts(deleteAccount);
      deleteAccount.deleteOne();
      res.status(200).send(`{"message": "conta ${deleteAccount.name}, ${deleteAccount.conta} numero: ${deleteAccount.conta} da agencia: ${deleteAccount.agencia} excluida com sucesso"}`);
    } catch (error) {
      next(error);
    }
  });
